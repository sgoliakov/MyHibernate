...
<body>
<c:choose>   <%--описание начальной страницы
если юзер не пустой --%>
    <c:when test="${not empty sessionScope.employee}">
        <p>Hello :${sessionScope.employee.firstName}</p><%--получаем имя --%>
...
<%-- если юзер пустой получаем и перебираем все доступные дни--%>
<c:choose>
